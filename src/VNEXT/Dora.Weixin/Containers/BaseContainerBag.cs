...
namespace Dora.Weixin.Containers
{
    ...
    public class BaseContainerBag : BindableBase, IBaseContainerBag
    {
        ...
        private void BaseContainerBag_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            var mqKey = DoraMessageQueue.GenerateKey("ContainerBag", sender.GetType(), containerBag.Key, "UpdateContainerBag");

            //获取对应Container的缓存相关

            //加入消息队列，每过一段时间进行自动更新，防止属性连续被编辑，短时间内反复更新缓存。
            DoraMessageQueue mq = new DoraMessageQueue();
            mq.Add(mqKey, () =>
            {
                ...
                //cacheKey形如:Container:Dora.Weixin.MP.Containers.AccessTokenBag:wx669ef95216eef885
