        ...
        public override Weixin.MP.Entities.IResponseMessageBase OnEvent_ClickRequest(RequestMessageEvent_Click requestMessage)
        {
            ...
            switch (requestMessage.EventKey)
            {
                ...
                case "SubClickRoot_Music":
                    {
                        //上传缩略图
                        var accessToken = Weixin.MP.Containers.AccessTokenContainer.TryGetAccessToken(appId, appSecret);
                        var uploadResult = Weixin.MP.AdvancedAPIs.MediaApi.UploadTemporaryMedia(accessToken, UploadMediaFileType.thumb,
                                                                     "");
                ...
                case "SubClickRoot_Image":
                    {
                        //上传图片
                        var accessToken = Weixin.MP.Containers.AccessTokenContainer.TryGetAccessToken(appId, appSecret);
                        var uploadResult = Weixin.MP.AdvancedAPIs.MediaApi.UploadTemporaryMedia(accessToken, UploadMediaFileType.image,
                                                                     "");
                        //设置图片信息
                ...
                case "GetNewMediaId"://获取新的MediaId
                    {
                        ...
                        try
                        {
                            var result = Weixin.MP.AdvancedAPIs.MediaApi.UploadForeverMedia(appId, "");
