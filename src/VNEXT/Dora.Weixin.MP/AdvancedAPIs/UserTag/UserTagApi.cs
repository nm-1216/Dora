...
/*----------------------------------------------------------------
    ...
    修改描述：修改命名空间
              其改为Dora.Weixin.MP.AdvancedAPIs    
...
using Dora.Weixin.MP.AdvancedAPIs.UserTag;
    ...
    public class UserTagApi
    {
        ...
        public static TagJson Get(string accessTokenOrAppId)
        {
            return ApiHandlerWapper.TryCommonApi(accessToken =>
            {
                ...
                return Dora.Utilities.HttpUtility.Get.GetJson<TagJson>(url);
        ...
        public static async Task<TagJson> GetAsync(string accessTokenOrAppId)
        {
            return await ApiHandlerWapper.TryCommonApiAsync(async accessToken =>
            {
                var urlFormat = Config.ApiMpHost + "/cgi-bin/tags/get?access_token={0}";
                var url = string.Format(urlFormat, accessToken);
                return await Dora.Utilities.HttpUtility.Get.GetJsonAsync<TagJson>(url);
