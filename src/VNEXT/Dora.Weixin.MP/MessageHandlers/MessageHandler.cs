...
namespace Dora.Weixin.MP.MessageHandlers
{
    /// <summary>
    /// 微信请求的集中处理方法
    /// 此方法中所有过程，都基于Dora.Weixin.MP的基础功能，只为简化代码而设。
    /// </summary>
        ...
        where TC : class, IMessageContext<IRequestMessageBase, IResponseMessageBase>, new()
    {
        ...
        /// 这里是Dora.Weixin.MP，根据请求消息半段在ResponseDocument基础上是否需要再次进行加密（每次获取重新加密，所以结果会不同）
        /// </summary>
