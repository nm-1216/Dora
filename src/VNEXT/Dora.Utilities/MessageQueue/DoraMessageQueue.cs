namespace Dora.Utilities.MessageQueue
{
    ...
    /// 消息队列
    /// </summary>
    public class DoraMessageQueue
    {
        /// <summary>
        /// 队列数据集合
        /// </summary>
        private static Dictionary<string, DoraMessageQueueItem> MessageQueueDictionary = new Dictionary<string, DoraMessageQueueItem>(StringComparer.OrdinalIgnoreCase);
        ...
        /// <returns></returns>
        public DoraMessageQueueItem GetItem(string key)
        {
            lock (MessageQueueSyncLock)
            {
        ...
        public DoraMessageQueueItem Add(string key, Action action)
        {
            lock (MessageQueueSyncLock)
            {
                //if (!MessageQueueDictionary.ContainsKey(key))
                ...
                var mqItem = new DoraMessageQueueItem(key, action);
